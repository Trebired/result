...
async function loadConfig(projectRoot = process.cwd(), options: LoadResultConfigOptions = {}): Promise<LoadedResultConfig> {
  ...
  if (!await pathExists(configPath)) throw new Error(`result config was not found: ${configPath}`);
  return loadedConfig(configPath, readSourceConfig(await fsPromises.readFile(configPath, "utf8"), configPath));
}

function loadConfigSync(projectRoot = process.cwd(), options: LoadResultConfigOptions = {}): LoadedResultConfig {
  const root = path.resolve(projectRoot);
  const configPath = options.configPath
  ? path.resolve(root, options.configPath)
  : findConfigSync(options.searchFrom || root, root);
  if (!configPath) return handleMissingConfig(options);
  if (!fs.existsSync(configPath)) throw new Error(`result config was not found: ${configPath}`);
  return loadedConfig(configPath, readSourceConfig(fs.readFileSync(configPath, "utf8"), configPath));
}

function loadCachedConfigSync(projectRoot = process.cwd()): NormalizedResultConfig {
  const root = path.resolve(projectRoot);
...
function resetConfigCacheForTests(): void {
  cachedConfigs = new Map<string, LoadedResultConfig>();
}
...
async function findConfig(startDir = process.cwd(), boundaryDir?: string): Promise<string|null> {
  ...
  for (;; ) {
    const candidate = path.join(current, RESULT_PROJECT_CONFIG_PATH);
...
function handleMissingConfig(options: LoadResultConfigOptions): LoadedResultConfig {
  if (options.defaultIfMissing === false) throw new Error("result config was not found");
  return missingConfig();
}

function missingConfig(): LoadedResultConfig {
...
function readSourceConfig(source: string, configPath: string): ResultConfig {
  const candidate = runConfigSource(source, configPath);
  if (!candidate || typeof candidate !== "object" || Array.isArray(candidate)) {
    throw new Error(`result config must default-export an object: ${configPath}`);
  }
  return candidate as ResultConfig;
}
...
function runConfigSource(source: string, configPath: string): unknown {
  try {
    ...
    throw new Error(`result config failed to load: ${configPath}: ${reason}`);
  }
}
...
function toRuntimeConfigSource(source: string, configPath: string): string {
  ...
  if (runtimeSource === withoutImports || /\bexport\b/u.test(runtimeSource)) {
    throw new Error(`result config only supports a default export: ${configPath}`);
  }
