...
- Updated the logger-adapter dependency so result logging uses the current shared adapter release.
...
- Added public `.trebired/result/config.ts` support through `@trebired/result/config`.
- Result responders and tracing now merge package config defaults before caller options.
